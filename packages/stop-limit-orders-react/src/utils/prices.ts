...
const INPUT_FRACTION_AFTER_FEE = (bpsFee?: Percent) =>
  ONE_HUNDRED_PERCENT.subtract(bpsFee ?? THIRTY_BIPS_FEE);

// computes realized lp fee as a percent
export function computeRealizedLPFeePercent(
  trade: V2Trade<Currency, Currency, TradeType>,
  bpsLpFee?: number | string
): Percent {
      ...
      (currentFee: Percent): Percent =>
        currentFee.multiply(
          INPUT_FRACTION_AFTER_FEE(
            bpsLpFee
              ? new Percent(JSBI.BigInt(bpsLpFee), JSBI.BigInt(10000))
              : undefined
          )
        ),
      ONE_HUNDRED_PERCENT
